...
describe UserServices::Querying do
  let(:alices_aspect) { alice.aspects.where(name: "generic").first }
  let(:eves_aspect) { eve.aspects.where(name: "generic").first }
  let(:bobs_aspect) { bob.aspects.where(name: "generic").first }
  ...
  describe "#visible_shareable_ids" do
    ...
    it "includes the reshare id on type reshare" do
    ...
    it "does not include the reshare id on type statusMessage" do
      post = bob.post(:status_message, text: "hey", public: true, to: bobs_aspect.id)
      reshare = bob.post(:reshare, root_guid: post.guid, to: bobs_aspect)
    ...
    it "does not contain duplicate posts" do
      ...
      expect(bob.aspects_with_person(alice.person)).to contain_exactly(bobs_aspect, bobs_other_aspect)
  ...
  describe "#visible_shareables" do
    it "never contains posts from people not in your aspects" do
      create(:status_message, public: true)
    ...
    context "with many posts" do
      ...
      it "works" do
        # The set up takes a looong time, so to save time we do several tests in one
        expect(bob.visible_shareables(Post).length).to eq(15) # it returns 15 by default
        expect(bob.visible_shareables(Post).map(&:id)).to eq(bob.visible_shareables(Post, by_members_of: bob.aspects.map {|a|
                                                                                                           a.id
                                                                                                         }).map(&:id)) # it is the same when joining through aspects

        # checks the default sort order
        expect(bob.visible_shareables(Post).sort_by {|p|
                 p.created_at
               }.map {|p| p.id }).to eq(bob.visible_shareables(Post).map {|p|
                                          p.id
                                        }.reverse) # it is sorted updated_at desc by default

        # It should respect the order option
        opts = {order: "created_at DESC"}
        expect(bob.visible_shareables(Post,
                                      opts).first.created_at).to be > bob.visible_shareables(Post, opts).last.created_at

        # It should respect the order option
        opts = {order: "updated_at DESC"}
        expect(bob.visible_shareables(Post,
                                      opts).first.updated_at).to be > bob.visible_shareables(Post, opts).last.updated_at

        # It should respect the limit option
        opts = {limit: 40}
        expect(bob.visible_shareables(Post, opts).length).to eq(40)
        expect(bob.visible_shareables(Post,
                                      opts).map(&:id)).to eq(bob.visible_shareables(Post, opts.merge(by_members_of: bob.aspects.map {|a|
                                                                                                                      a.id
                                                                                                                    })).map(&:id))
        expect(bob.visible_shareables(Post, opts).sort_by {|p|
                 p.created_at
               }.map {|p| p.id }).to eq(bob.visible_shareables(Post, opts).map {|p|
                                          p.id
                                        }.reverse)
        ...
        expect(bob.visible_shareables(Post, opts).map {|p|
                 p.id
               }).to eq(bob.visible_shareables(Post, opts.merge(by_members_of: bob.aspects.map {|a|
                                                                                 a.id
                                                                               })).map {|p|
                          p.id
                        })
        ...
        expect(bob.visible_shareables(Post, opts).map {|p|
                 p.id
               }).to eq(bob.visible_shareables(Post, limit: 40)[15...30].map {|p|
                          p.id
                        }) # pagination should return the right posts
  ...
  context "with two users" do
    describe "#people_in_aspects" do
      ...
      it "returns local/remote people objects for a users contact in each aspect" do
        local_user1 = create(:user)
        local_user2 = create(:user)
        remote_person = create(:person)
  ...
  context "contact querying" do
    ...
    describe "#aspects_with_person" do
      let(:connected_person) { bob.person }

      it "returns the aspects with given contact" do
